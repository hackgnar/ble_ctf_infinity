...
uint16_t blectf_handle_table[PCAP_WRITE_IDX_NB];
...
static const esp_gatts_attr_db_t gatt_db[PCAP_WRITE_IDX_NB] =
{
    // Service Declaration
    [PCAP_WRITE_IDX_SVC]        =
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        case ESP_GATTS_REG_EVT:{
            ...
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
    #endif
            esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, PCAP_WRITE_IDX_NB, SVC_INST_ID);
            if (create_attr_ret){
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            if (!param->write.is_prep){
                ...
                if (param->write.handle == blectf_handle_table[PCAP_WRITE_IDX_CHAR_WRITE_WARP]+1){
                    esp_restart();
                }
		if (param->write.handle == blectf_handle_table[PCAP_WRITE_IDX_CHAR_READ_FLAG]+1){
		    if (param->write.value[0] == 0x12 && 
				    param->write.value[1] == 0x12 && 
				    param->write.value[2] == 0x12 && 
				    param->write.value[3] == 0x12 && 
				    param->write.value[4] == 0x12 && 
				    param->write.value[5] == 0x22 )
		    {
		        //TODO: set flag value in handle
                        esp_ble_gatts_set_attr_value(blectf_handle_table[PCAP_WRITE_IDX_CHAR_READ_FLAG]+1, sizeof(flag_pcap_write_value)-1, (uint8_t *)flag_pcap_write_value);
		    }else{
                        esp_ble_gatts_set_attr_value(blectf_handle_table[PCAP_WRITE_IDX_CHAR_READ_FLAG]+1, sizeof(pcap_url)-1, (uint8_t *)pcap_url);
		    } 
                }
	    }
            else{
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            ...
            else if (param->add_attr_tab.num_handle != PCAP_WRITE_IDX_NB){
                ESP_LOGE(GATTS_TABLE_TAG, "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to IDX_NB(%d)", param->add_attr_tab.num_handle, PCAP_WRITE_IDX_NB);
            }
            else {
                ESP_LOGI(GATTS_TABLE_TAG, "create attribute table successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
                memcpy(blectf_handle_table, param->add_attr_tab.handles, sizeof(blectf_handle_table));
                esp_ble_gatts_start_service(blectf_handle_table[PCAP_WRITE_IDX_SVC]);
            }
            break;
        }
