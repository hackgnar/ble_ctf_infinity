...
static uint16_t heart_rate_handle_table[MITM_PSK_PAIR_REQ_IDX_NB];
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            esp_ble_gatts_create_attr_tab(heart_rate_gatt_db, gatts_if,
                                      MITM_PSK_PAIR_REQ_IDX_NB, HEART_RATE_SVC_INST_ID);
            break;
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ...
            if (param->create.status == ESP_GATT_OK){
                if(param->add_attr_tab.num_handle == MITM_PSK_PAIR_REQ_IDX_NB) {
                    memcpy(heart_rate_handle_table, param->add_attr_tab.handles,
                    sizeof(heart_rate_handle_table));
                    esp_ble_gatts_start_service(heart_rate_handle_table[MITM_PSK_PAIR_REQ_IDX_SVC]);
                }else{
                    ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to MITM_PSK_PAIR_REQ_IDX_NB(%d)",
                         param->add_attr_tab.num_handle, MITM_PSK_PAIR_REQ_IDX_NB);
                }
            }else{
