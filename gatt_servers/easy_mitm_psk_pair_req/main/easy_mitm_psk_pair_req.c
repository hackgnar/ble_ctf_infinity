...
#include "easy_mitm_psk_pair_req.h"
...
static uint16_t heart_rate_handle_table[EASY_MITM_PSK_PAIR_REQ_IDX_NB];
...
static const char mitm_read_value[] = "Connect with pin 0000";
static char warp_value[] = "Write to goto scoreboard";

static char flag_easy_mitm_psk_pair_req_value[] = "12345678901234567890";

static const esp_gatts_attr_db_t heart_rate_gatt_db[EASY_MITM_PSK_PAIR_REQ_IDX_NB] =
{
    // Heart Rate Service Declaration
    [EASY_MITM_PSK_PAIR_REQ_IDX_SVC]                    =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ,
      sizeof(uint16_t), sizeof(heart_rate_svc), (uint8_t *)&heart_rate_svc}},

    // Body Sensor Location Characteristic Declaration
    [EASY_MITM_PSK_PAIR_REQ_IDX_BOBY_SENSOR_LOC_CHAR]  =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      CHAR_DECLARATION_SIZE,CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read}},

    // Body Sensor Location Characteristic Value
    [EASY_MITM_PSK_PAIR_REQ_IDX_BOBY_SENSOR_LOC_VAL]   =
    ...
    [EASY_MITM_PSK_PAIR_REQ_IDX_HR_CTNL_PT_CHAR]          =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      CHAR_DECLARATION_SIZE,CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read}},

    // Heart Rate Control Point Characteristic Value
    [EASY_MITM_PSK_PAIR_REQ_IDX_HR_CTNL_PT_VAL]             =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&heart_rate_ctrl_point, ESP_GATT_PERM_READ_ENC_MITM,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(flag_easy_mitm_psk_pair_req_value)-1, (uint8_t *)flag_easy_mitm_psk_pair_req_value}},

    // warp - return to scoreboard
    [EASY_MITM_PSK_PAIR_REQ_IDX_WRITE_WARP_CHAR]  =
    ...
    // warp - return to scoreboard
    [EASY_MITM_PSK_PAIR_REQ_IDX_WRITE_WARP_VAL]   =
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  ///the app will receive this evt when the IO  has Output capability and the peer device IO has Input capability.
        ESP_LOGI(GATTS_TABLE_TAG, "ESP_GAP_BLE_PASSKEY_NOTIF_EVT");
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            esp_ble_gatts_create_attr_tab(heart_rate_gatt_db, gatts_if,
                                      EASY_MITM_PSK_PAIR_REQ_IDX_NB, HEART_RATE_SVC_INST_ID);
            break;
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            if (!param->write.is_prep){
                ...
                if (param->write.handle == heart_rate_handle_table[EASY_MITM_PSK_PAIR_REQ_IDX_WRITE_WARP_CHAR]+1){
                    esp_restart();
                }
            }
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ...
            if (param->create.status == ESP_GATT_OK){
                if(param->add_attr_tab.num_handle == EASY_MITM_PSK_PAIR_REQ_IDX_NB) {
                    memcpy(heart_rate_handle_table, param->add_attr_tab.handles,
                    sizeof(heart_rate_handle_table));
                    esp_ble_gatts_start_service(heart_rate_handle_table[EASY_MITM_PSK_PAIR_REQ_IDX_SVC]);
                }else{
                    ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to EASY_MITM_PSK_PAIR_REQ_IDX_NB(%d)",
                         param->add_attr_tab.num_handle, EASY_MITM_PSK_PAIR_REQ_IDX_NB);
                }
            }else{
