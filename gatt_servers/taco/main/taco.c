...
uint16_t blectf_handle_table[TACO_IDX_NB];
...
static const char docs_value[] = "Feed me three tacos!";
static const char warp_value[] = "write here to goto to scoreboard";

static char flag_taco_value[] = "12345678901234567890";
static char flag_taco_tmp_value[] = "";

/* Full Database Description - Used to add attributes into the database */
static const esp_gatts_attr_db_t gatt_db[TACO_IDX_NB] =
{
    // Service Declaration
    [TACO_IDX_SVC]        =
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_READ_FLAG, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(flag_taco_tmp_value)-1, (uint8_t *)flag_taco_tmp_value}},
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        case ESP_GATTS_REG_EVT:{
            ...
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
    #endif
            esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, TACO_IDX_NB, SVC_INST_ID);
            if (create_attr_ret){
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            if (!param->write.is_prep){
                ...
                if (param->write.handle == blectf_handle_table[TACO_IDX_CHAR_WRITE_WARP]+1){
                    esp_restart();
                }
                if (param->write.handle == blectf_handle_table[TACO_IDX_CHAR_READ_DOCS]+1 ) {
                    if( strcmp(writeData, "🌮🌮🌮") == 0){
                        esp_ble_gatts_set_attr_value(blectf_handle_table[TACO_IDX_CHAR_READ_FLAG]+1, sizeof(flag_taco_value)-1, (uint8_t *)flag_taco_value );
                    }
                    esp_ble_gatts_set_attr_value(blectf_handle_table[TACO_IDX_CHAR_READ_DOCS]+1, sizeof(docs_value)-1, (uint8_t *)docs_value );
                }
            }
            else{
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            ...
            else if (param->add_attr_tab.num_handle != TACO_IDX_NB){
                ESP_LOGE(GATTS_TABLE_TAG, "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to IDX_NB(%d)", param->add_attr_tab.num_handle, TACO_IDX_NB);
            }
            else {
                ESP_LOGI(GATTS_TABLE_TAG, "create attribute table successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
                memcpy(blectf_handle_table, param->add_attr_tab.handles, sizeof(blectf_handle_table));
                esp_ble_gatts_start_service(blectf_handle_table[TACO_IDX_SVC]);
            }
            break;
        }
